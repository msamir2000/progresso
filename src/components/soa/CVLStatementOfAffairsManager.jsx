...
import { Plus, ChevronDown, ChevronRight, Trash2, Lock, Unlock, FileDown, RefreshCw, Users } from 'lucide-react';
...
export default function CVLStatementOfAffairsManager({ caseId, onUpdate }) {
  ...
  // Save function for immediate database updates
  const saveToDatabase = async (dataToSave) => {
    ...
    try {
      ...
      if (soaRecords.length > 0) {
        ...
        console.log('✅ CVL Statement of Affairs saved immediately');
      } else {
        ...
        console.log('✅ CVL Statement of Affairs created immediately');
      }
    } catch (error) {
      console.error('❌ Error saving CVL Statement of Affairs:', error);
      throw error;
    }
  };
  ...
  useEffect(() => {
    ...
    const loadSoaData = async () => {
      ...
      try {
        ...
        // Fetch creditors and employees with delay to avoid rate limits
        fetchPromises.push(
          new Promise(resolve => setTimeout(async () => {
            try {
              const result = await base44.entities.Creditor.filter({ case_id: caseId });
              resolve(result);
            } catch (err) {
        ...
        fetchPromises.push(
          new Promise(resolve => setTimeout(async () => {
            try {
              const result = await base44.entities.Employee.filter({ case_id: caseId });
              resolve(result);
            } catch (err) {
        ...
        console.log('Successfully loaded SoA data:', {
          soaRecords: soaRecords?.length || 0,
          hasCase: !!caseResponse,
          creditors: allCreditorsResponse?.length || 0,
          employees: allEmployeesResponse?.length || 0
        });
        ...
        if (soaRecords.length > 0) {
          ...
          console.log('=== CVL SoA - LOADING VERSION ===');
          ...
          // Detailed logging for each uncharged asset
          loadedData.scheduleA?.globalAssets?.uncharged?.forEach((asset, idx) => {
            console.log(`  Uncharged Asset ${idx + 1}:`);
            console.log('    - ID:', asset.id);
            console.log('    - Name/Description:', asset.name);
            console.log('    - Account Code:', asset.account_code);
            console.log('    - Book Value:', asset.book_value);
            console.log('    - ETR Value:', asset.etr_value);
            console.log('    - Description field:', asset.description);
          });
          console.log('=================================');
  ...
  const updateAsset = useCallback(async (sectionId, assetType, index, field, value) => {
    if (isLocked) return;
  ...
  const addAsset = async (sectionId, assetType) => {
    if (isLocked) return;
  ...
  const removeLastAsset = async (sectionId, assetType) => {
    if (isLocked) return;
  ...
  const handleSyncFromCreditors = async () => {
    ...
    setIsLoading(true);
    try {
      const [allCreditorsResponse, allEmployeesResponse] = await Promise.all([
...
<body>
  ...
  if (isLoading) {
    return <div className="p-4 text-center">Loading Statement of Affairs...</div>;
  }
          ...
          onClick={async () => {
            try {
              ...
              if (soaRecords.length > 0) {
                ...
                alert('Statement of Affairs created and saved to database successfully!');
              }
            } catch (error) {
